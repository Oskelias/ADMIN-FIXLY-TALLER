  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success: "border-fixly-green-500 bg-fixly-green-50 text-fixly-green-700",
        warning: "border-fixly-yellow-500 bg-fixly-yellow-50 text-fixly-yellow-700",
        destructive:
          "destructive group border-fixly-red-500 bg-fixly-red-50 text-fixly-red-700",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-gray-500/50 opacity-0 transition-opacity hover:text-gray-900 focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-fixly-red-300 group-[.destructive]:hover:text-fixly-red-50 group-[.destructive]:focus:ring-fixly-red-400 group-[.destructive]:focus:ring-offset-fixly-red-600",
      className
    )}
    toast-close=""
    {...props}
